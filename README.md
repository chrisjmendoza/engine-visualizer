...
- **Side-by-side comparison.** Render two engines at a shared millimeter scale, driven by the same crank angle, with a difference table showing signed percentage deltas for every metric.
...
- Single cylinder per engine; multi-cylinder layouts (inline, V, flat) are a planned milestone.
