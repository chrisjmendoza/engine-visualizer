...
export function encodeShareState(state: ShareState): string {
  ...
  if (!state.isPlaying) {
    const degrees = ((state.crankAngleRad % TWO_PI) + TWO_PI) % TWO_PI;
    params.set("angle", formatNumber((degrees * 180) / Math.PI));
  }
...
export function decodeShareState(query: string): PartialShareState {
  ...
  if (rawRpm !== null) {
    ...
    if (Number.isFinite(rpm) && rpm >= 0 && rpm <= 10_000) {
      state.rpm = rpm;
    }
  }
  ...
  if (rawAngle !== null) {
    const degrees = Number(rawAngle);
    if (Number.isFinite(degrees)) {
      const radians = (degrees * Math.PI) / 180;
      state.crankAngleRad = ((radians % TWO_PI) + TWO_PI) % TWO_PI;
