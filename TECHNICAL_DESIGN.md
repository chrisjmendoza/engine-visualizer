...
**How the two combine.** Rotating a mechanism by `β` puts its crankpin where an unrotated one would draw `ψ − β`, so two cylinders share one physical crankpin exactly when `φ − β` agrees: `φ_bank1 = φ_bank0 + bankAngle`. A narrow-angle V achieves even firing by staggering the pair's pins a further angle apart instead: a small stagger (30°, on a 90° V6) leaves enough pin overlap to cut as a true split pin — one journal machined as two overlapping halves — but a 60° stagger (`v6-60`) does not, so those pins are cast as two separate crankpins joined by a flying arm. A boxer shares no pins at all — opposed cylinders sit on separate throws 180° apart, which with bores 180° apart makes their phases equal, so the opposed pistons move outward together. That is the real difference between a flat-4 boxer and a 180° V, and only the boxer is modeled.
...
- **Flying-arm V** (`v6-60`): each cylinder has its own crankpin rather than sharing one with its bank partner; the pair's pins are offset a further 60° apart — too little overlap for a true split pin, which is why a flying-arm web joins two separate journals here instead. **Both are real and both are drawn.** This is the case a "V engines share pins" shortcut would get wrong, and it is why the decision is made from the pin geometry rather than from the layout kind.
...
**What the override cannot change, and why.** Both cylinders of a throw pair are rotated by the same angle, so the pair's relative geometry is untouched: whether they share a crankpin or sit on antipodal ones is a fact about the engine, `sharesCrankpin` reads the real `bankOffsetRad`, and the one-crank-per-throw drawing decision comes out identical with the preference on or off. So does the grouping into throws, so does `throwIndex`. The comparison arrangement rule is likewise untouched: it keys off how many cylinders are on stage (`visibleCylinders`), never off how they are turned, so drawing lone cylinders upright cannot flip a side-by-side pair into a stacked one. Every one of those is asserted directly in `sceneGeometry.test.ts`.

### Amendment — per-cylinder four-stroke phase, and the firing tint

_Added 2026-08-31. Extends §24a and the four-stroke overlay (`src/engine/cycle.ts`); supersedes the paragraphs above wherever the two disagree._

Each cylinder is now tinted by what it is doing in the four-stroke cycle — **red while firing, blue while exhausting, untinted through intake and compression** — so an inline-6 or a cross-plane V8 shows its firing sequence travelling down the engine. That is the one thing a static diagram cannot convey, and it is the payoff for carrying real firing orders in the layout roster.

**The cycle phase is engine-layer truth.** `cycle.ts` previously answered only "which stroke is cylinder 0 in", from a crank angle plus the revolution-parity bit. Every other cylinder needs its own answer, and it is **not** derivable from `crankPhaseRad`: crank phase is modulo 360° while the cycle is 720°, so two cylinders whose pistons move identically — an inline-4's cylinders 1 and 4 — can be a full revolution apart in the cycle. The missing input is the firing order, which the layout already carries as real published data.

**The convention, stated once so it is never re-guessed.** `strokePhaseAt` reads crank angle 0 at parity 0 as the start of **cylinder 0's intake stroke**, which puts cylinder 0's power stroke at [2π, 3π) and its firing event at cycle angle **2π**. That is the fixed point. Every other cylinder is placed against it by shifting the engine's cycle angle back by that cylinder's own firing angle:

```
cylinderCycleAngleRad(cyl) = engineCycleAngle − cylinderFiringAngleRad(cyl)   (mod 4π)
```

`cylinderFiringAngleRad` (in `engineLayout.ts`) is `firingSequenceRad` re-indexed **by cylinder index** instead of by position in the firing order, built once per layout and frozen — the per-frame caller must never walk the sequence again (§18). Cylinder 0's entry is 0 by construction, so `cylinderStrokePhaseAt(layout, 0, θ)` reduces to exactly `strokePhaseAt(θ)` and the stroke badge and the scene can never disagree. The derivation is also self-consistent with crank geometry: cylinder _k_ reaches its own cycle angle 2π at engine cycle angle `firingAngle_k + 2π`, which modulo one revolution is `firingAngle_k` — an occurrence of that cylinder's own TDC. Every cylinder therefore starts its power stroke at its own top dead center, as a real one does.

**Why this is over-tested on purpose.** A wrong per-cylinder offset produces a tint rhythm that looks entirely plausible on screen and describes an engine that does not exist. So `cycle.test.ts` sweeps the full 720° cycle at 0.05° resolution, records the order in which cylinders actually enter the power phase, and asserts it equals the layout's stored `firingOrder` — for every layout in the roster, not just the named five. Around it: every power entry lands on that cylinder's own TDC (a check against crank geometry, independent of the firing order the offsets came from); the swept gaps reproduce `firingIntervalsRad`, including `v6-90-odd`'s real 150°/90° alternation, which must never flatten to an even 120°; inline-4's cylinders 1 and 4 stay exactly two strokes apart despite sharing a crank throw (the case a crank-phase-only derivation fails outright); the phase split stays as even as the cylinder count allows; and cylinder 0 agrees with `strokePhaseAt` at every sampled angle.

**The tint is presentation, and is decided in one place** — the same discipline `drawnRotationRad` follows. `src/scene/chamberTint.ts` owns the colors (`SCENE_COLORS.chamberFiring` / `chamberExhaust`, both at roughly `clearance`'s lightness and both clearly distinct from the accent orange the moving joints are marked with, so a tinted chamber is never mistaken for a highlighted pin) and the rule that intake and compression stay neutral. Nothing in `src/engine/` knows a color exists. `CylinderGuide` groups the three meshes that make up the combustion chamber — the two bore walls beside the clearance volume and the head face closing its top — behind a stage-owned ref; the swept walls below and the deck above stay structural.

**Per-frame rules.** The tint would be a rerender per cylinder several times per revolution if it were React state, which §11 forbids as firmly as a per-frame setter, so the frame loop mutates the materials directly. `PlacedEngine` carries the shared frozen `EngineLayoutDefinition` so the loop can ask the engine layer for a phase without rebuilding a layout; `cylinderStrokePhaseAt` reads a frozen lookup and returns a string literal, so nothing allocates; and `applyChamberPhase` tracks the last phase written per cylinder and **writes only on a change** — a handful of writes per 720° cycle rather than one per cylinder per frame. The 720° position comes from the loop's existing revolution-parity bit, the one notion of cycle position there is; engine B works identically from its own angle and its own parity.
...
### Amendment — each stacked row is centered on its own crank span

_Added 2026-08-31. Supersedes the shared slot-0 crank center of the amendment immediately above; everything else about stacking, including each row keeping its own spacing, stands._

**Each row of a stacked comparison is centered on the stack's center line by the midpoint of its own crank span** — the X of its first throw's crank center to the X of its last throw's. Rows of unequal length now sit squarely one above the other instead of the shorter one trailing off the left end of the frame.

**The span is measured from crank centers, not from drawn bounds.** A row's drawn bounds swell with bank tilt and with the `uprightFlatEngines` quarter turn, so centering on them would let a V's wide tilted throws drag the row off-center relative to the crankshaft it is actually built on. The crankshaft is the engine's spine and is what should line up. Since slots are evenly spaced, the span is exactly `(slots − 1) × spacing`, so a **one-slot row** — a single-cylinder view, or a one-cylinder architecture — has a zero-width span and simply puts its one crank center on the line. The degenerate case falls out of the arithmetic; nothing is special-cased and nothing is divided by the slot count.

**What this costs, and what it does not.** It gives up the cylinder-1-over-cylinder-1 datum the previous anchor provided; that is a deliberate call made after seeing the lopsided result on screen. It does **not** cost the same-layout column alignment: two engines of the same layout and the same dimensions measure the same spacing and the same slot count, so their spans are equal, and equal spans centered on one line coincide slot for slot. That is verified rather than assumed. It also does not re-space anything — centering only translates a row, so every row is still exactly the row that engine would draw alone, and the shared zoom (§12.2) is untouched.

**What it buys, measured.** On the pairing this was reported against — the two GT-Rs, an RB26DETT inline-6 stacked with a VR38DETT `v6-60` — the inline-6's crank span sat **53.1mm** left of the frame's center line under the shared anchor (its row spanning x ∈ [−497.4, 391.2] of a framed union of [−628.2, 628.2]) and sits on it exactly now, the row spanning a symmetric [−444.3, 444.3]. On the LS7-dimensioned inline-6 / V8-cross pair from the previous amendment's worked example, the same offset was **368.4mm** and is now zero. The framed union is unchanged in both cases (1256.4mm and 2165.9mm) — the longer row already set it — so the balance is gained at no cost in zoom.

**In the code.** `firstCenterForCrankSpanCenteredAt(row, centerXMm)` in `src/scene/sceneGeometry.ts` is the whole of the rule, and `placeStacked` calls it once per row with `centerXMm = 0`; the two rows' extents are no longer combined to pick a shared anchor. `placeRow` is unchanged, as are `measureRow`, `rowExtentsAt`, the vertical gap, the label bands, the side-by-side arrangement, and the single-engine path — the last two asserted against their arithmetic written out independently, for every layout in the roster.
